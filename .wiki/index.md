...
*   **`test_mapping.js`**: A dedicated script for programmatically loading a trained TensorFlow.js model and testing it against a predefined set of binary input-output test cases. It reports the accuracy of the model's predictions in a tabular format. Includes the same Node.js v24 polyfill for `util.isNullOrUndefined` as `index.js`.
...
*   **`util` (Node.js built-in)**: A polyfill for `util.isNullOrUndefined` is included in both `index.js` and `test_mapping.js` to ensure compatibility with Node.js v24, where this function was removed.

### 4. API Entry Points and Main Functions

The project has three primary execution paths:

#### 4.1. Logic Mapping CLI (`index.js`)
...
*   **Key Functions**:
    *   `util.isNullOrUndefined`: A polyfill for compatibility with Node.js v24.