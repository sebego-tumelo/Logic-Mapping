...
*   **`child_process` (Node.js built-in)**: Specifically `execSync`, used by `sync-wiki.js` to execute shell commands such as `git rev-parse --short HEAD` (to get commit SHAs), `npx repomix` (to generate code snapshots), and `git diff` (to get code changes).
*   **`path` (Node.js built-in)**: Used by `sync-wiki.js` for constructing and normalizing file paths (e.g., `path.join`).
...
#### 4.2. Wiki Synchronization Script (`sync-wiki.js`)

*   **Entry Point**: `node --env-file=.env sync-wiki.js` or `npm run update` (requires `GEMINI_API_KEY` to be set in a `.env` file).
*   **Main Functions/Flows**:
